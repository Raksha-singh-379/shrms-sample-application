...
type RestOf<T extends IState | NewState> = Omit<T, 'createdOn' | 'lastModified'> & {
...
export class StateService {
  ...
  protected convertDateFromClient<T extends IState | NewState | PartialUpdateState>(state: T): RestOf<T> {
    return {
      ...state,
      createdOn: state.createdOn?.toJSON() ?? null,
  ...
  protected convertDateFromServer(restState: RestState): IState {
    return {
      ...restState,
      createdOn: restState.createdOn ? dayjs(restState.createdOn) : undefined,
