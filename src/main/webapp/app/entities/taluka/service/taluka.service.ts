...
type RestOf<T extends ITaluka | NewTaluka> = Omit<T, 'createdOn' | 'lastModified'> & {
...
export class TalukaService {
  ...
  protected convertDateFromClient<T extends ITaluka | NewTaluka | PartialUpdateTaluka>(taluka: T): RestOf<T> {
    return {
      ...taluka,
      createdOn: taluka.createdOn?.toJSON() ?? null,
  ...
  protected convertDateFromServer(restTaluka: RestTaluka): ITaluka {
    return {
      ...restTaluka,
      createdOn: restTaluka.createdOn ? dayjs(restTaluka.createdOn) : undefined,
