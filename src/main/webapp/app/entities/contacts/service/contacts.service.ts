...
type RestOf<T extends IContacts | NewContacts> = Omit<T, 'lastModified' | 'createdOn'> & {
...
export class ContactsService {
  ...
  protected convertDateFromClient<T extends IContacts | NewContacts | PartialUpdateContacts>(contacts: T): RestOf<T> {
    return {
      ...
      createdOn: contacts.createdOn?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restContacts: RestContacts): IContacts {
    return {
      ...
      createdOn: restContacts.createdOn ? dayjs(restContacts.createdOn) : undefined,
    };
  }
