...
type RestOf<T extends IDepartment | NewDepartment> = Omit<T, 'lastModified' | 'createdOn'> & {
...
export class DepartmentService {
  ...
  protected convertDateFromClient<T extends IDepartment | NewDepartment | PartialUpdateDepartment>(department: T): RestOf<T> {
    return {
      ...
      createdOn: department.createdOn?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restDepartment: RestDepartment): IDepartment {
    return {
      ...
      createdOn: restDepartment.createdOn ? dayjs(restDepartment.createdOn) : undefined,
    };
  }
