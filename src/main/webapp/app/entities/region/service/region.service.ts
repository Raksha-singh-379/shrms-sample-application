...
type RestOf<T extends IRegion | NewRegion> = Omit<T, 'createdOn' | 'lastModified'> & {
...
export class RegionService {
  ...
  protected convertDateFromClient<T extends IRegion | NewRegion | PartialUpdateRegion>(region: T): RestOf<T> {
    return {
      ...region,
      createdOn: region.createdOn?.toJSON() ?? null,
  ...
  protected convertDateFromServer(restRegion: RestRegion): IRegion {
    return {
      ...restRegion,
      createdOn: restRegion.createdOn ? dayjs(restRegion.createdOn) : undefined,
